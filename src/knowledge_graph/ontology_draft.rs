...
pub fn induce_from_terms(
    ...
    let mut warnings = vec![
        "Schema induction creates reviewable type drafts only; it does not promote types.".into(),
...
mod tests {
    ...
    fn induction_creates_object_drafts_with_auditable_provenance_only() {
        ...
        assert!(draft
            .warnings
            .iter()
            .any(|warning| warning.contains("ActionType")));
        assert!(draft.object_types.iter().any(|object| object.id == "Asset"));
        assert!(draft
            .warnings
            .iter()
            .any(|warning| warning.contains("too broad")));
    }
