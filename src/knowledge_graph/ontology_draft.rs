...
pub struct TypeDraftBundle {
    ...
    /// How an induction request produced this bundle. Schema adapters that
    /// operate on explicit machine-readable input intentionally omit it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub provenance: Option<TypeDraftProvenance>,
...
/// Auditable, caller-supplied metadata for a pre-kernel schema-induction run.
/// This is draft metadata only; it never reaches the production ontology.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct TypeDraftProvenance {
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// A source document supplied to a schema-induction request. Its content is
/// used only to derive candidates; its stable id is retained as provenance.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InductionDocument {
    pub id: String,
    pub text: String,
}
...
/// Produce reviewable object-type candidates from an operator/LLM supplied
/// terminology list and conservative repeated-title-case terms in source
/// documents. It deliberately emits no links or actions: relationships and
/// executable behavior require separate, explicit human design.
pub fn induce_from_terms(
    candidate_terms: Vec<String>,
    documents: &[InductionDocument],
    provenance: TypeDraftProvenance,
) -> Result<TypeDraftBundle, String> {
    if provenance.rule_version.trim().is_empty() {
        return Err("rule_version must not be empty".into());
    }
    if provenance.source_document_ids.is_empty() {
        return Err("at least one source document id is required".into());
    }

    let title_case = Regex::new(r"\b[A-Z][A-Za-z0-9]*(?:\s+[A-Z][A-Za-z0-9]*){0,2}\b")
        .expect("constant title-case regex is valid");
    let mut terms = candidate_terms;
    let mut occurrences = std::collections::HashMap::<String, usize>::new();
    for document in documents {
        for term in title_case.find_iter(&document.text) {
            *occurrences.entry(term.as_str().to_string()).or_default() += 1;
        }
    }
    terms.extend(
        occurrences
    ...
    let mut warnings = vec![
        "Schema induction creates reviewable type drafts only; it does not promote types."
            .into(),
        "No LinkType or ActionType is generated by schema induction.".into(),
    ];
    for term in terms {
        let label = term.trim();
        let id = normalize_type_id(label);
        ...
        if label.split_whitespace().count() == 1
            && matches!(
                ...
                "data" | "entity" | "item" | "object" | "record" | "thing"
            )
        {
            warnings.push(format!(
                "Candidate `{label}` may be too broad; narrow it before promotion."
            ));
        }
        object_types.push(ObjectType {
            iri: ev(&id),
            label: label.to_string(),
            description: "Draft inferred from supplied terminology or repeated document mentions; review before promotion.".into(),
            icon: "Lightbulb".into(),
            color: "amber".into(),
            ...
            properties: vec![PropertySpec {
                ...
                prop_type: PropertyType::String,
                required: true,
                description: Some("Provisional draft identifier; review before promotion.".into()),
                enum_values: vec![],
            }],
            id,
        });
    ...
    if object_types.is_empty() {
        return Err("no usable candidate terms were supplied or found in source documents".into());
    }
    object_types.sort_by(|a, b| a.id.cmp(&b.id));
    Ok(TypeDraftBundle {
        object_types,
...
mod tests {
    ...
    #[test]
    fn induction_creates_object_drafts_with_auditable_provenance_only() {
        let provenance = TypeDraftProvenance {
            model_version: Some("candidate-generator-2026-09".into()),
            rule_version: "terminology-v1".into(),
            source_document_ids: vec!["doc-1".into()],
        };
        let draft = induce_from_terms(
            vec!["Asset".into(), "record".into()],
            &[InductionDocument {
                id: "doc-1".into(),
                text: "Asset Asset".into(),
            }],
            provenance.clone(),
        )
        .unwrap();
        assert_eq!(draft.provenance, Some(provenance));
        assert!(draft.link_types.is_empty());
        assert!(draft.suggested_links.is_empty());
        assert!(draft.warnings.iter().any(|warning| warning.contains("ActionType")));
        assert!(draft.object_types.iter().any(|object| object.id == "Asset"));
        assert!(draft.warnings.iter().any(|warning| warning.contains("too broad")));
    }
