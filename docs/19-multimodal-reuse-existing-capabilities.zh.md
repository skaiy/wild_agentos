...
| 当前快照 | WAO `v0.7.0` · OIDC · 通用业务 Agent |
...
| **§5 技能图谱** | 识图/OCR/VL 补全视为 **AtomicSkill 或 MCP 封装**；文本抽取用 `AlternativeLink` 降级；经验用 KnowledgeFragment | 在 Core 写死业务专用 OCR 逻辑 |
...
**已交付 P0（v0.6.2）：** 既有 vision mount 是支持的图片路径。视觉能力不可用时默认
fail closed；显式的兼容降级路径可观测，图片限制返回 `413`，绝不静默丢弃图片。
业务客户端应配置 model resources 和 mounts，而不是修改 Core。
...
### P0 — 已于 v0.6.2 交付：正道（优先配置 + 契约硬化）

| ID | 事项 | 类型 | 验收 |
|---|---|---|---|
| P0-1 | 已文档化 `images`、`vision`/`chat` mounts、上限和错误码 | 文档 | 与代码一致 |
| P0-2 | 已支持 model resources 和 `model_mounts.vision` | **配置** | 业务 Agent 可挂载 VL 模型 |
| P0-3 | vision 不可用时行为可观测 | 策略 | 不会出现假识图成功 |
| P0-4 | payload 上限返回可配置的 `413` 响应 | 已硬化 | 超限可测 |
| P0-5 | OIDC + isolation + 带图已有正/负例 CI 覆盖 | 测试 | 无 claims 仍返回 `401` |
...
已交付的 P0 契约是支持路径。业务客户端可以在本仓库外使用应用专用 fallback，
但它不替代通用 chat 契约。
...
| 建议切片 | 内容 |
|---|---|
| 已于 v0.6.2 交付 | P0：文档、VL 挂载、静默丢图治理、413、CI |
...
> 业务客户端消费此契约；领域行为与 HITL 均在本仓库外。P0 契约已于 **v0.6.2** 交付。
...
4. ~~目标版本：`0.6.x` 热修还是 `0.7.0`？~~ **已交付：** **v0.6.2**。
