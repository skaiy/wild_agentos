...
|---------|--------------|------------------------------|
| **v0.6.0** | **2026-09-08** | **Online Corpus Job + Watcher**<br>• Adds authenticated, claims-scoped, idempotent online corpus jobs and default-enabled watchers for configured changes or incremental deltas; watcher polling and enqueueing can be explicitly disabled per deployment.<br>• The runner requires approval-held entity-resolution suggestions, stages evidence, and never auto-merges entities, promotes ontology, or materializes production data.<br>• Adds bounded provenance and scoped observability for job state, retries, saturation, source versions/content digests, canonicalization, quality/review, and ER suggestions; transient sidecar failures retry at most three times.<br>• Adds fail-closed isolation and production-write CI for online jobs, runners, and watchers. The companion Admin job list is delivered separately. |
...
- **v0.6.0 — done:** authenticated, claims-scoped online corpus jobs and default-enabled corpus watchers now orchestrate the existing KE primitives with idempotency, retry/backpressure, provenance, and observable job state. Watchers can be explicitly disabled by deployment configuration. The runner requires approval-held entity-resolution suggestions and stages evidence; production ontology promotion, entity-resolution merge, and materialization remain human-governed. Fail-closed isolation CI proves online jobs, runners, and watchers cannot write production through unauthenticated, invalid, cross-scope, or failed paths. The companion Admin job list is delivered separately.
